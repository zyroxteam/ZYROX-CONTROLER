...
async function bootstrap() {
  ...
  const adminBotConfig = await Setting.findOne({ key: 'admin_bot_config_version' }).lean();
  if (Number(adminBotConfig?.value || 0) < 2 && config.adminIds.length) {
    ...
    await Setting.findOneAndUpdate({ key: 'admin_bot_config_version' }, { value: 2 }, { upsert: true });
    console.log(`Owner chat migrated to configured admin: ${config.ownerChatId}`);
  } else if (savedOwner?.value && /^\d+$/.test(String(savedOwner.value))) {
  ...
  await User.updateMany({ role: 'admin', telegramId: { $nin: config.adminIds } }, { $set: { role: 'user' } });
  ...
  app.get('/api/v1/devices/:deviceId/next-command', asyncRoute(authenticateDevice), asyncRoute(async (req, res) => {
    ...
    return res.json({ ok: true, authorized: true, linked: true, maintenance: false, autoSixColours, command: command ? { id: String(command._id), colour: command.colour, dice: command.dice, createdAt: command.createdAt } : null });
  }));
