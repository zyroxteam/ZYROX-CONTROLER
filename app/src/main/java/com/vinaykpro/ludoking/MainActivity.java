...
public class MainActivity extends AppCompatActivity {
    ...
    class Dice
    {
        ...
        Dice(ImageView diceImgView,int nop,String color)
        {
            ...
            diceImgView.setOnLongClickListener(view -> {
                if (!isRolling) ControllerDialog.show(MainActivity.this);
                return true;
            });
        ...
        void roll() {
            ...
            diceHandler.postDelayed(() -> {
                int ch = ControlClient.get(MainActivity.this).takePendingDice(currentPlayerColor);
                if (ch < 1 || ch > 6) ch = (int) Math.ceil(Math.random()*6);
                switch (ch) {
                    case 1:
