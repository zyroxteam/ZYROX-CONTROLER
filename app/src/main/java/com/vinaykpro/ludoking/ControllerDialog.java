...
public final class ControllerDialog {
    ...
    public static void show(Activity activity){
        ...
        connect.setOnClickListener(v->{loading(status,"Opening dice control bot…");connect.setEnabled(false);client.register(r->{connect.setEnabled(true);render(status,instruction,r);String link=r.botLink.isEmpty()?client.getBotLink():r.botLink;try{activity.startActivity(new Intent(Intent.ACTION_VIEW, Uri.parse(link)));}catch(Exception e){copy(activity,client.getDeviceId());Toast.makeText(activity,"Telegram open nahi hua; @ZyroxLudoKingbot open karein",Toast.LENGTH_LONG).show();}});});
